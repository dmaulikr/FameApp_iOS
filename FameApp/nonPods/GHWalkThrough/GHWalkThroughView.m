...
- (void) orientFooter
{
    if (self.walkThroughDirection == GHWalkThroughViewDirectionVertical) {
        ...
        if (isRotated) {
            ...
            self.skipButton.frame = CGRectMake(self.frame.size.width - 100, self.pageControl.frame.origin.y - ((30 - self.pageControl.frame.size.height)/2), 100, 30);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (page < [self.dataSource numberOfPages]-1) {
        [self setCloseTitle:@""];
    }
    else {
        [self setCloseTitle:@"LET'S GO"];
    }
}
